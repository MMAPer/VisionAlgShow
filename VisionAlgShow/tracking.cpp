...
void addCboxItem2(QComboBox *target, vector<string> items);
...
//open file
void tracking::on_btn_open_clicked()
{
    ...
    if (capture.isOpened()){
        ...
        if(fileSuffix == "jpg" || fileSuffix == "png" || fileSuffix == "JPEG" ){
           ...
           // cv::resize(image, image, Size(kInpWidth, kInpHeight));
            QImage img = tracking::Mat2QImage(image);
...
// opencv Mat to Image
QImage tracking::Mat2QImage(cv::Mat cvImg)
{
    QImage qImg;
...
void tracking::bgModel()
{
    ...
    if (videoFlag==1){
        ...
        image=tracking::Mat2QImage(fgmask);
        ui->label_play->setPixmap(QPixmap::fromImage(image));
        ui->label_play->setAlignment(Qt::AlignCenter);
...
void tracking::on_btn_faster_rcnn_clicked()
{
...
void tracking::on_btn_ssd_clicked()
{
    ...
    if (videoFlag==1){
        ...
        image=tracking::Mat2QImage(frame);
        ui->label_play->setPixmap(QPixmap::fromImage(image));
        ui->label_play->setAlignment(Qt::AlignCenter);
        if (waitKey(1) >= 0) break;
        ...
        if(!ssd_frame.data)
            ...
            QImage ssd_image;
            ssd_image=tracking::Mat2QImage(ssd_frame);
...
void tracking::on_btn_hog_clicked()
{
...
void tracking::on_btn_yolo_clicked()
{
    ...
    for(;;)
    {
        ...
        QImage img=tracking::Mat2QImage(frame);
        ui->label_play->setPixmap(QPixmap::fromImage(img));
        ui->label_play->setAlignment(Qt::AlignCenter);
    }
...
void tracking::on_btn_dpm_clicked()
{
